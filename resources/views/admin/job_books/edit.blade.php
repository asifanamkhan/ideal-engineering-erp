                    ...
                    <div class="col-md-12 row">
                        <div class="col-md-6">
                            <div class="mb-2">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#customerModal" style="white-space: nowrap;">
                        ...
                        <div class="col-md-6">
                            <div class="mb-2">
                                ...
                                <input type="text" name="vehicle_registration_no" class="form-control" value="{{ $jobBook->vehicle_registration_no }}" placeholder="Enter vehicle registration number">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-2">
                                ...
                                <select name="job_status" class="form-control">
                                    ...
                                    <option value="delivered" {{ $jobBook->job_status == 'delivered' ? 'selected' : '' }}>Delivered</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-2">
                                ...
                                @if($jobBook->documents)
                                    <small class="text-muted d-block mt-1">Current: <a href="{{ asset($jobBook->documents) }}" target="_blank">View Document</a></small>
                                @endif
                            </div>
                        </div>
                        <div class="col-md-6">
                ...
                <div class="row mb-3 align-items-end">
                    <div class="col-md-8">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#jobDescriptionModal" style="white-space: nowrap;">
                                <i class="fas fa-plus-circle me-1"></i> New
                            </button>
                        </div>
                    </div>
                ...
                <div class="table-responsive mt-3">
                    <table class="table table-sm table-bordered" id="descriptionsTable">
                        ...
                        <tbody id="descriptionsTableBody">
                            @foreach($existingDescriptions as $index => $desc)
                            <tr class="description-row">
                                <td>{{ $index + 1 }}</td>
                                <td>
                                    {{ $desc->description }}
                                    <input type="hidden" name="descriptions[{{ $index }}][description_id]" value="{{ $desc->job_description_id }}">
                                    @if($desc->id)
                                    <input type="hidden" name="descriptions[{{ $index }}][id]" value="{{ $desc->id }}">
                                    @endif
                                </td>
...
<script>
    $(document).ready(function() {
        ...
        let rowCounter = {{ count($existingDescriptions) }};
        let existingItems = @json($existingDescriptions);

        // Load existing descriptions
        $.each(existingItems, function(index, item) {
            descriptionItems.push({
                id: item.id,
                description_id: item.job_description_id,
        ...
        function renderDescriptionTable() {
            ...
            $.each(descriptionItems, function(index, item) {
                var idField = item.id ? `<input type="hidden" name="descriptions[${index}][id]" value="${item.id}">` : '';
                var row = `
                    <tr class="description-row">
                        <td>${index + 1}</td>
                        <td>
